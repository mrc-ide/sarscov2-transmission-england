...
ld_dates <- list( ld1 = "2020-03-25", ld2 = "2020-11-05")
...
admitted <- extract_value("admitted", paper_date, agg_samples$england) +
  extract_value("new", paper_date, agg_samples$england)
...
mean_ci(start_date$south_east - start_date$london)
```
...
```{r}
prop_chr_deaths <- extract_value("deaths_comm", end_first_wave,
                                 agg_samples$england) /
  extract_value("deaths", end_first_wave, agg_samples$england)

paste0(round(mean_ci(prop_chr_deaths), 3) * 100, "%")

sum(agg_data$england$full$deaths_comm, na.rm = TRUE) /
  sum(agg_data$england$full$deaths, na.rm = TRUE)
```
...
peak_diff <- function(sample, end_date) {
  chw <- calc_peak(sample, "deaths_comm_inc", end_first_wave)
...
```{r}
lapply(ld_dates, extract_Rt_est, agg_Rt_outputs$england, "eff_Rt_general")
```

$R_t$ in second lockdown

```{r}
lapply(ld_dates, extract_Rt_est, agg_Rt_outputs$england, "Rt_general")
```
...
reduction_in_severity <- function(ifr, date1, date2) {
  ret <- 1 - (ifr$IFR_AR_all[ifr$date == date2, ] /
    ifr$IFR_AR_all[ifr$date == date1, ])
  mean_ci(ret)
}

round(t(sapply(ifr_t, reduction_in_severity, end_first_wave, paper_date)) * 100, 1)
```
...
```{r}
extract_params(samples, what = sprintf("C_%s", 1:2), digits =7)
```

Transistion probabilities

```{r}
v <- c("hosp_sympt", "death_comm", "ICU_hosp", "death_hosp_D", "death_ICU",
       "death_stepdown")
...
```{r}
extract_params(samples, what = sprintf("mu_%s", c("death_hosp", "ICU_hosp")),
               digits = 2)
```
...
```{r}
extract_params(samples, what = c("prop_noncovid_sympt", "rho_pillar2_tests"),
               digits = 4)
```
